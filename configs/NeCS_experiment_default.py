...
config_dict = dict(
    ...
    model_params=dict(
        ...
        dist_thresh=5., # latent dim
        sample_from_pred=True, # whether to sample from the predicted distribution (True during inference)
        ...
        scale_decoder_sigma=0.1, # scale the sigma of the decoder (if not fixed std)
    ),
    ...
    increase_rollout_epochs=[51, 101, 151, 201, 226, 251, 276, 301, 326], # when to increase the rollout length during training
    beta=1.0, # beta in beta-vae
    ...
    experiment={'state_dict_fname': 'NeCS_state_dict.pt'},
)
